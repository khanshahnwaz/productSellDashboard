...
const BarChart = () => {
  ...
  return (
    <div className=" w-2/3 bg-white shadow rounded ">
       <div className='p-5'>
                <p className='font-bold text-xl'>Overview</p>
                 <small>Monthly Earning</small>
            </div>
      <div className=" p-4  flex flex-wrap justify-between relative top-1/2">
        {data.map(item => (
          <div className=" items-center mb-4 " key={item.month}>
            <div className={ `h-full w-2 relative`}>
              <div className={` ${item.h} ${item.curr?'bg-blue-600':'bg-blue-400'} w-8 rounded-md absolute bottom-0`}></div>
            </div>
